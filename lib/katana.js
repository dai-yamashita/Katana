...
if (staticsEnabled) {
  ...
  var Static = new (require('node-static').Server)({
    ...
    serverInfo: conf.serverInfo || ('Katana v'+ App.info.version),
...
App.handleRequest = function(request, response) {
  ...
  if (staticsEnabled && (Path.normalize(request.path) + '/').indexOf(staticsPath) === 0) {
...
function importFiles(type, fn, next) {
  ...
  Async.each(App._modules, function(module, cb) {
    var path = module === 'app' ? 'app/'+ type : 'modules/'+ module + '/'+ type;

    requireDir(App.root + path, fn, function(items) {
