...
module.exports = {
  ...
  encode64: function encode64(str) {
	...
	load: function load(path, done, callback, dir, basedir) {
	  ...
	  Fs.exists(path + dir, function(exists) {
	    ...
	    Fs.readdir(path + dir, function(error, files) {
	        ...
	        var dirname = dir.replace(new RegExp('^'+ basedir.replace('\\', '\\\\') +''), '');
	...
	// parse range header of request
	parse_range: function(size, str) {
		var valid = true;
	...
	request_modified: function(request, response, headers) {
		var headers        = headers || response._headers || {};
		var modified_since = request.headers['if-modified-since'];
		var last_modified  = headers['last-modified'];
	  var none_match     = request.headers['if-none-match'];
	  var etag           = headers['etag'];
	
	  if (none_match) { none_match = none_match.split(/ *, */); }
	
		if (none_match && etag && ~none_match.indexOf(etag)) {
		  return false;
		}
	    ...
	    if (!isNaN(modified_since.getTime())) {
	      if (last_modified <= modified_since) return false;
	    }
	  }
